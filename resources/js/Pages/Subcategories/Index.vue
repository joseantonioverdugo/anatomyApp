					...
					<table class="w-full whitespace-no-wrap">
						...
						<tbody class="bg-white divide-y">
							<tr v-for="subcategory in subcategories.data" :key="subcategory.id" class="text-gray-700">
								...
								<td class="px-4 py-3 text-sm">
									<NavLink :href="route('subcategories.show', subcategory.id)">
										<SecondaryButton>
											<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
								...
								<td class="px-4 py-3 text-sm">
									<WarningButton @click="openModalForm(1, subcategory)">
										<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
								...
								<td class="px-4 py-3 text-sm">
                                    <DangerButton @click="openModalDelete(subcategory)">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
	...
	<Modal :show=showModalForm @close=closeModalForm>
		<div class="p-6">
            ...
            <div class="mt-6 mb-6 space-y-6 max-w-xl">
                ...
                <PrimaryButton @click="storeSubcategory">Save</PrimaryButton>
            </div>
        </div>
	...
	<Modal :show="showModal" @close="closeModalDelete">
		<div class="p-6">
			<p class="text-2xtext-gray-500">
				...
				<PrimaryButton @click="deleteSubcategory">Delete</PrimaryButton>
		</div>
		<div class="m-6 flex justify-end">
...
const openModalForm = (type, category) => {
    ...
    if(type === 1) {
        title.value = 'Editar Subcategoría'
        form.id = category.id
        form.name = category.name
    } else {
        title.value = 'Crear Subcategoría'
        form.id = null
        form.name = ''
    }
}
...
const storeSubcategory = () => {
    if(form.id){
        form.put(route('subcategories.update', form.id))
		closeModalForm()
    }else{
...
const openModalDelete = (category) => {
	...
	title.value = 'Eliminar Subcategoría';
	form.id = category.id;
	form.name = category.name;
}
...
const deleteSubcategory = () => {
	form.delete(route('subcategories.destroy', form.id))
	closeModalDelete()
}
</script>